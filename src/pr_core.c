...
void priority_scheduling(Process* proc_list, int proc_qnt) {
...
static Process* schedule_next_process(Process* proc_list, int list_size) {
  ...
  int i;
  int indx = -1;
  float priority_max = 20;
  for(i = 0; i < list_size; i++) {
    if((proc_list[i].step == READY || proc_list[i].step == BLOCKED) && proc_list[i].priority < priority_max) {
      indx = i;
      priority_max = proc_list[i].dt;
    }
  }
  if(indx == -1){
    return NULL;
...
static void reassign_busy_core(Process* proc, Process* master, Core* core_vector, unsigned int internal_cores) {
  int i;
  int indx = -1;
  float priority_min = -20;
  for(i = 0; i < internal_cores; i++) {
    if(core_vector[i].working_proc->priority > priority_min){
      priority_min = core_vector[i].working_proc->priority;
      indx = i;
    }
  }
  if(priority_min > proc->priority){
    core_vector[indx].working_proc->step = BLOCKED;
    notify_blocked_worker(indx , core_vector[indx].working_proc->name , proc->name, priority_min, proc->dt);
