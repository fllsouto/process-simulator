...
void shortest_remaining_time_first(Process* proc_list, int proc_qnt) {
...
static Process* schedule_next_process(Process* proc_list, int list_size) {
  ...
  for(i = 0; i < list_size; i++) {
    if((proc_list[i].step == READY || proc_list[i].step == BLOCKED) && proc_list[i].dt < dt_min) {
      indx = i;
...
static void reassign_busy_core(Process* proc, Process* master, Core* core_vector, unsigned int internal_cores) {
  ...
  float dt_max = 0;
  for(i = 0; i < internal_cores; i++) {
    if(core_vector[i].working_proc->dt > dt_max){
      dt_max = core_vector[i].working_proc->dt;
      indx = i;
    }
  }
  if(dt_max > proc->dt){
    core_vector[indx].working_proc->step = BLOCKED;
    notify_blocked_worker(indx , core_vector[indx].working_proc->name , proc->name, dt_max, proc->dt);
