...
static void reassign_busy_core(Process*, Process*, Core*, unsigned int);
...
void realtime_scheduling_with_hard_deadline(Process* proc_list, int proc_qnt) {
...
static void* do_master_routine(void* args) {
  ...
  Process* master = ((Process*) args);
  Core* core_vector;
  int free_core = -1;
  unsigned int internal_cores = sysconf(_SC_NPROCESSORS_CONF);
  core_vector = create_core_vector(internal_cores);
  master->context_swap = 0;
  processed_threads = 0;
  clock_gettime(CLOCK_REALTIME, &start_time);
  start_cpu = clock();
  
  while(processed_threads < master->list_size){
    Process* selected_p;
    fetch_ready_process(master);
    selected_p =  schedule_next_process(master->list, master->list_size);
    free_core = get_free_core(core_vector, internal_cores);
    if(selected_p != NULL){
      if(free_core != NOT_FREE_CORE){
        assign_free_core(selected_p, &core_vector[free_core]);
      } else if(selected_p->step != BLOCKED){
        reassign_busy_core(selected_p, master, core_vector, internal_cores);
      }
    }
  ...
  calculate_time_elapsed(master, &end_time, &start_time);
  
  free(core_vector);
  core_vector = NULL;
  
  notify_context_swap_total(master->context_swap);
  return NULL;
}
...
static Process* schedule_next_process(Process* proc_list, int list_size) {
  ...
  int i;
  int indx = -1;
  float deadline_min = FLT_MAX;
  for(i = 0; i < list_size; i++) {
    if((proc_list[i].step == READY || proc_list[i].step == BLOCKED) && proc_list[i].deadline < deadline_min) {
      indx = i;
      deadline_min = proc_list[i].dt;
    }
  }
  if(indx == -1){
    return NULL;
...
static void do_thread_job(Process* proc) {
  ...
  while(proc->dt > 0 && proc->step == RUNNING) {
...
static void reassign_busy_core(Process* proc, Process* master, Core* core_vector, unsigned int internal_cores) {
  int i;
  int indx = -1;
  float deadline_max = 0;
  for(i = 0; i < internal_cores; i++) {
    if(core_vector[i].working_proc->deadline > deadline_max){
      deadline_max = core_vector[i].working_proc->deadline;
      indx = i;
    }
  }
  if(deadline_max > proc->deadline){
    core_vector[indx].working_proc->step = BLOCKED;
    notify_blocked_worker(indx , core_vector[indx].working_proc->name , proc->name, deadline_max, proc->dt);
    core_vector[indx].working_proc = NULL;
    assign_free_core(proc, &core_vector[indx]);
    master->context_swap++;
  }
}