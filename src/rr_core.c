...
static void release_core_from_quantum_break(Process*, Core*, unsigned int );
...
void round_robin(Process* proc_list, int proc_qnt) {
...
static void* do_master_routine(void* args) {
  ...
  while(processed_threads < master->list_size){
    Process* selected_p;
    release_core_from_quantum_break(master, core_vector, internal_cores);
    fetch_ready_process(master);
    if(queue_is_empty(head) == FALSE) {
      free_core = get_free_core(core_vector, internal_cores);
      if(free_core != NOT_FREE_CORE){
        selected_p =  remove_from_queue(head);
...
static void fetch_ready_process(Process* master) {
  ...
  for(i = 0; i < master->list_size; i++){
    ...
    if(absolute_time >= aux->t0 && aux->step == WAIT){
      ...
      aux->step = READY;
      head = insert_on_queue(aux, head);
/*      print_as_a_queue(head);*/
    }
  }
}
...
static void release_core_from_quantum_break(Process* master, Core* core_vector, unsigned int internal_cores) {
  int i;
  int k = 0;
  for(i = 0; i < internal_cores; i++) {
    if(core_vector[i].busy == TRUE && core_vector[i].working_proc != NULL && core_vector[i].working_proc->quantum_break == TRUE){
      core_vector[i].working_proc->step = BLOCKED;
      core_vector[i].working_proc->quantum_break = FALSE;

      head = insert_on_queue(core_vector[i].working_proc, head);
      /*print_as_a_queue(head);
      notify_new_blocked_worker_on_queue(i, core_vector[i].working_proc->name, core_vector[i].working_proc->dt);*/
  ...
  /*printf("Released [%d] cores from quantum_break\n", k);*/
}